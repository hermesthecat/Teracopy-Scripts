...
REM === Copy to Target Folder ===
echo.
echo %COLOR_CYAN%[INFO] Starting copy to: "%target_path%"%COLOR_RESET%
start /wait "" "%teracopy%" copy "%outputDir%\*" "%target_path%" /Close
if errorlevel 1 (
    echo %COLOR_RED%[ERROR] Copy failed.%COLOR_RESET%
    pause
    exit /b 1
)
echo %COLOR_CYAN%[INFO] Copy complete.%COLOR_RESET%
